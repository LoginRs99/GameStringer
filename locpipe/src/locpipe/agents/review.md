...
5. Terminology and character voice (glossary is authoritative; speaker's register if one applies). Formality rule: %%REGISTER_INSTRUCTION%%
6. Target grammar on placeholders: ensure target language grammar (suffixes, particles, prepositions, plurals) attaches cleanly without corrupting placeholder ASCII tokens (e.g. for Hungarian, use hyphens like {item}-t; for Japanese, particles like は, が, を; for Latin/Germanic languages, prepositions and spacing).
