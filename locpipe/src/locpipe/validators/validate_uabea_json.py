...
def validate_file(
    ...
    def _validate_dict_object(obj: dict):
        if "m_Script" in obj and isinstance(obj["m_Script"], str):
            script_text = obj["m_Script"]
    ...
    if isinstance(data, dict):
        _validate_dict_object(data)
    elif isinstance(data, list):
        if not data:
            info.append("UABEA JSON array is empty.")
        for item in data:
            if isinstance(item, dict):
                _validate_dict_object(item)
    else:
        critical.append(f"Expected JSON object or array in '{path.name}', got {type(data).__name__}")
